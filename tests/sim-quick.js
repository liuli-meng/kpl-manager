// 平衡性快速门禁（CI 用）：三档开局 + 2017 时代档各 50 赛季（约 35 秒），关键比率越界即失败。
// 区间按 sim.js 大样本校准值 ± 抽样噪声设置（见 README「平衡门禁」）——拦的是结构性漂移，不是微观波动。
// 用法：node tests/sim-quick.js [--n=50] [--seed=<整数|random>]
...
// 2026-09 复测（n=100，种子 42/12345/777）：AG 夺冠 92%~94% · 自建季后赛 47%~58% · UUG 53%~64%。
// 区间按「大样本中值 ± 抽样噪声」设——门禁拦的是结构性漂移（AG 场场夺冠 / 自建常年垫底），
// 不是 n=30 的掷硬币抖动。旧带宽 ≤95%（AG）/≤55%（自建）在真值 93%/52% 下假红率过高（AG 侧 ~38%）。
const BANDS = {
  '自建新队': r => (r.po/N >= 0.15 && r.po/N <= 0.72) || `自建季后赛率 ${(r.po/N*100).toFixed(0)}% 越界 [15%,72%]（基线 ~50%：拦结构性崩坏/碾压）`,
  'AG豪门':   r => r.my/N <= 0.98 || `AG 夺冠率 ${(r.my/N*100).toFixed(0)}% 越界 ≤98%（基线 ~93%：银河战舰允许强，但不允许场场全胜）`,
  'UUG弱旅':  r => r.po/N <= 0.78 || `UUG 季后赛率 ${(r.po/N*100).toFixed(0)}% 越界 ≤78%（基线 ~55%~60%：弱旅不应稳定碾压）`,
