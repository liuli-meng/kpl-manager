...
fs.readdirSync(path.join(__dirname, '..', 'src', 'js')).forEach(f => {
  fs.readFileSync(path.join(__dirname, '..', 'src', 'js', f), 'utf8')
    .split('\n').forEach((line, i) => {
...
// ──  旧调用形式（字符串）也必须能推进——归一化的直接证据 ──
{
  ...
  const r = vm.runInContext(`(function(){
    ...
    return {after:JSON.stringify(S.playoff), ls1:(localStorage.getItem(slotKey())||'')!==ls0, a:nextAction(S)};
  })()`, dom);
