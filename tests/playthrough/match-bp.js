...
async function main(){
  ...
  try {
    ...
    const afterCont = await page.evaluate(() => ({
      modalOn: !!document.querySelector('#app-modal.on'),
      // 弹窗还开着的话，先看是谁开的：nextDay 会正常触发随机事件/董事会弹窗，那不是 bug
      modalWho: (() => {
        const on = [...document.querySelectorAll('.modal-bg.on')].map(m => m.id);
        const body = document.querySelector('#app-modal.on .modal');
        return { on, head: body ? (body.innerText || '').replace(/\n+/g, ' | ').slice(0, 140) : null };
      })(),
      series: !!S.series,
      matchIdx: S.matchIdx,
      phase: S.phase,
    ...
    // 继续之后 nextDay 会正常弹出「3 步上手」引导（开局前几日的既定行为），只排除引导类弹窗
    const guideModal = /步上手|新手引导|完整引导/.test((afterCont.modalWho && afterCont.modalWho.head) || '');
    ok('continue closes modal & advances', !afterCont.series && (!afterCont.modalOn || guideModal),
      JSON.stringify(afterCont) + (guideModal ? ' [引导弹窗，视为正常]' : ''));
    await shot(page, 'mp_07_after_continue');
    report.shots.push('mp_07_after_continue.png');
    // 引导弹窗会盖住后面的 BP 流程（day 2 触发与否看 RNG），先按玩家操作把它跳过，保证用例确定性
    await page.evaluate(() => {
      if (document.querySelector('#app-modal.on') && typeof tourSkip === 'function') tourSkip();
