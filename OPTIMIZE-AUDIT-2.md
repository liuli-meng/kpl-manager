...
| 2 | **读档不是瓶颈**：热身后中位 **5.1 ms** | ✅ 结案：无需动 |
| 3 | **选手名单索引化是伪优化** | ✅ 结案：已否决 |
| 4 | **`tests/dev/` 诊断脚本路径断裂** | ✅ 已修 + 常驻门禁 |
| 5 | 三份副本只剩主线活跃 | ✅ 结案：`refactor` 已删；`rewrite` 留作参考（`STATUS.md` 已标注，不合回） |
| 6 | 存档体积 `.skill` / `history.logs` | ✅ 结案：不建议动，议题关闭 |

> 2026-09-25 复核：六条均已落地或明确结案，无待办。
