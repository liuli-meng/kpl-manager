...
function draftTeamRosterCount(s,team){
 if(team===s.teamName)return (s.players||[]).filter(p=>!(p.kjia>0)).length;
...
function draftRepair(s,d){
 ...
 // done 与 phase 不一致：面板显示进行中却一个按钮都不给
...
function draftAiAuction(s){ // AI 叫价直到轮到玩家或签位落定
 ...
 /* 只剩领先者还在场 → 成交。必须排在「轮到玩家就 break」之前：
    玩家叫价后领先的正是玩家本人，先 break 会让签位永不落定。 */
...
function draftBidRaise(s){
 ...
 const d=s.draft;
 if(!d||d.done||d.phase!=='auction')return;
 // 当前价必须是 d.bid（选秀自己的竞拍价）；误写 s.bid 会变成 NaN
...
function draftFaInit(p){
 ...
 p.freeAgent=true;
 try{p.signCost=Math.round(valueOf(overall(p))*0.58);}catch(e){p.signCost=50;}
...
function draftAiPick(s,team){
 ...
 // AI 点名必须以 def 入册：只 push 进 aiRosters 缓存，读档/赛季轮换后人会蒸发
...
function draftRegisterAiPick(s,team,p){
 ...
 const wouldGrow=cur.some(x=>x.pos===p.pos)?0:1;
...
function draftPick(s,id){
 ...
 d.slot++;
 draftNextSlot(s);
 if(!d.done){save();renderAll();} // 点完必须落盘+刷新，否则画面纹丝不动、刷新即丢
}
...
function draftPanelHtml(){
 ...
 <div class="hint" style="margin-bottom:8px">流程：先<strong>竞拍签位</strong>（前8签 ${DRAFT_BID_TOP}万起 / 第9签起 ${DRAFT_BID_REST}万起，加价 ${DRAFT_BID_STEP} 万；叫价受<strong>本队签位预算上限</strong>约束），拍到再<strong>点名</strong>。池子=训练营+K甲突出者（本届二队名次影响 K甲前三档：${kj.n}人 · 底子${kj.base}+）。<strong>不能选自家青训</strong>；自家苗子用训练页自留签（每季2个）；大名单上限 ${ROSTER_MAX} 人。</div>`;
 ...
 /* 整张卡可点：只把小按钮做成热区时，玩家点卡片本体没反应，会被当成「选不了人」。
    onclick 只挂在外层容器上，避免一次点击调用两次 draftPick。 */
