...
function pcard(p,extra){
  ...
  return `<div class="pcard ${ovrCls(o)}">
    ...
    ${energyBar(p)}
    <button class="btn sm" style="margin-top:6px" onclick="showCareer(findPlayerCard('${p.id}'))">${ic('doc',12)} 选手档案</button>
    ${extra||''}
  </div>`;
}
...
function showReplay(h){
  ...
  $('#app-modal-body').innerHTML=`
    <h2><span class="h-ic">${ic('film')}</span>复盘：${S.teamName} vs ${h.opp} <span class="tag">${h.stage} ${h.score}</span></h2>
...
function showCareer(p){
  ...
  $('#app-modal-body').innerHTML=`
    <h2><span class="h-ic">${ic('users')}</span>${p.name} 选手档案</h2>
...
function energyBar(p){
  ...
  return `<div class="pbar"><span>${ic('bolt',11)} 体力</span><div class="progress"><i style="width:${p.energy}%;background:${c}"></i></div><span>${p.energy}</span>
  <span style="margin-left:8px">${ic('flame',11)} 士气</span><div class="progress"><i style="width:${p.morale}%;background:${p.morale<40?'var(--red)':'var(--cyan)'}"></i></div><span>${p.morale}</span></div>`;
}
...
function renderHeader(){
  ...
  $('#header').innerHTML=`
    ...
    <div class="stats">
      ...
      <div class="stat">${ic('case',13)}<b>${sp.income}万/天</b><small>${sp.name}</small></div>
      ${S.coach?`<div class="stat gold">${ic('coach',13)}<b>${S.coach.name}</b><small>教练 +${S.coach.bonus}%</small></div>`:''}
    </div>
...
function renderClub(){
  ...
  <div class="banner">
    ...
    <div class="dim" style="font-size:12px">赛季${S.season} · 第${S.day}天 · ${PHASE_NAME[S.phase]||S.phase}${myGroup(S)?' · '+myGroup(S)+'组':''}${S.phase==='playoff'&&S.playoff?' · 双败淘汰':''}</div></div>
    <div style="margin-left:auto;text-align:right">
      <div class="gold" style="font-size:18px;font-weight:800">${fmt(S.fund)}万</div>
  ...
  if(S.phase==='r1'||S.phase==='r2'||S.phase==='r3'){
    ...
    if(S.preseason){
      ...
      html+=`<div class="panel">
        <h3><span class="h-ic">${ic('swap')}</span>赛前转会期 <span class="tag">剩余 ${S.transferWindow} 天 · 结束后开赛</span></h3>
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin:8px 0">${POS_ORDER.map(pos=>{
          ...
          return p?`<span class="tag">${POS[pos][1]} ${p.name} · 战力${playerPower(p,p.sig)}</span>`:`<span class="tag" style="color:var(--red)">⚠️ ${POS[pos][1]} 空缺！</span>`;
        }).join('')}</div>
      ...
      html+=`<div class="panel">
        <h3><span class="h-ic">${ic('play')}</span>下一场比赛 <span class="tag">${PHASE_NAME[S.phase]} · ${g}组 第${m.round}/${KPL.ROUNDS}轮</span></h3>
        <div class="match">
          <div class="vs"><div style="font-size:26px">${S.icon}</div><div><div class="tname">${S.teamName}</div><div class="power">战力 ${fmt(teamPower(S))}</div></div></div>
          <div style="color:var(--dim);font-weight:900">VS</div>
          <div class="vs" style="justify-content:flex-end;text-align:right"><div><div class="tname">${oppIcon} ${m.opp}</div><div class="power">战力 ${fmt(powerOf(S,m.opp))}</div></div></div>
        </div>
    ...
    html+=`<div class="panel"><h3><span class="h-ic">${ic('medal')}</span>卡位赛 <span class="tag">BO7 · 含巅峰对决</span></h3>
    ...
    html+=`<div class="panel center">
      <div style="font-size:40px;color:var(--gold)">${ic('trophy',38)}</div>
      <h3 style="justify-content:center">赛季${S.season} 总冠军：${S.playoff?S.playoff.champ:'—'}</h3>
      ${S.champion?'<div class="green" style="font-size:16px;font-weight:800;margin:8px 0">你是冠军！王朝就此建立！</div>':'<div class="dim">冠军属于对手，下赛季卷土重来！</div>'}
      <button class="btn gold mt12" onclick="newSeason(S)">${ic('play',14)} 开启新赛季（+130万资金）</button>
    </div>`;
  }else if(S.phase==='eliminated'){
    html+=`<div class="panel center">
      <div style="font-size:40px;color:var(--faint)">${ic('info',38)}</div>
      <h3 style="justify-content:center">本赛季止步</h3>
      <div class="dim" style="margin:8px 0">未能晋级后续阶段（B组后2名 / 卡位赛失利 / 季后赛出局）</div>
      <button class="btn gold mt12" onclick="newSeason(S)">开启新赛季（+130万资金）</button>
    </div>`;
  }
  // 今日行动
  html+=`<div class="panel">
    <h3><span class="h-ic">${ic('clock')}</span>今日行动 <span class="tag">每天可选择一项</span></h3>
    <div class="g2">
      <div class="pcard" style="border-color:var(--line)">
        <div style="font-weight:800;margin-bottom:6px">${ic('train',13)} 训练</div>
      ...
      <div class="pcard" style="border-color:var(--line)">
        <div style="font-weight:800;margin-bottom:6px">${ic('clock',13)} 休息</div>
  ...
  if(S.coach){
    const c=S.coach;
    html+=`<div class="panel"><h3><span class="h-ic">${ic('coach')}</span>主教练 <span class="tag">${c.rating||80}评分 · ${COACH_STYLE[c.style]}型</span></h3>
      <div class="sponsor"><span class="s-icon">${ic('coach',16)}</span>
...
function renderBiz(){
  ...
  let html=`<div class="panel"><h3><span class="h-ic">${ic('case')}</span>赞助商 <span class="tag">每日结算收入</span></h3>`;
  ...
  // 工资帽
  {
    ...
    html+=`<div class="panel"><h3><span class="h-ic">${ic('coin')}</span>工资帽 <span class="tag">KPL 联盟制度</span></h3>
  ...
  html+=`<div class="panel"><h3><span class="h-ic">${ic('trophy')}</span>荣誉室 <span class="tag">累计 ${champCount} 冠</span></h3>`;
  if(hon.length){
  ...
  if(S.history&&S.history.length){
    html+=`<div class="panel"><h3><span class="h-ic">${ic('film')}</span>比赛复盘 <span class="tag">最近 ${S.history.length} 场</span></h3>
...
function renderLineup(){
  ...
  let html=`<div class="panel"><h3><span class="h-ic">${ic('users')}</span>首发阵容 <span class="tag">${POS_ORDER.length}人</span></h3>
    <div class="dim" style="font-size:12px;margin-bottom:10px">总战力 <b class="cyan">${fmt(teamPower(S))}</b> · 总身价 <b class="gold">${fmt(ls.reduce((t,p)=>t+sellAskPrice(p),0))}万</b> · 士气均值 ${Math.round(ls.reduce((t,p)=>t+p.morale,0)/Math.max(1,ls.length))}% · 周薪合计 <b class="gold">${weeklyWage(S)}万</b></div>
  ...
  // 战队羁绊（上场选手触发，属于阵容维度）
  html+=`<div class="panel"><h3><span class="h-ic">${ic('link')}</span>战队羁绊 <span class="tag">同队上场生效</span></h3>`;
  if(bonds.length){
...
function renderMarket(){
  ...
  // 教练区
  let coachHtml=`<div class="panel ${foldCls('mcoach')}" data-fold="mcoach"><h3><span class="h-ic">${ic('coach')}</span>教练市场 <span class="tag">主教练决定全队战力</span></h3>`;
  if(S.coach){
    const c=S.coach;
    coachHtml+=`<div class="sponsor" style="border-color:var(--gold)">
      <span class="s-icon">${ic('coach',16)}</span>
  ...
  if(S.coachMarket.length){
    coachHtml+=`<div class="g3">${S.coachMarket.map(c=>{
      ...
      return `<div class="pcard ${ovrCls(c.rating||80)}" style="text-align:center">
        <div style="margin:6px 0;color:var(--faint)">${ic('coach',26)}</div>
    ...
    const listed=(S.listed||[]).map(item=>{
      ...
      return `<div class="match" style="margin-bottom:6px;padding:8px 10px">
        ...
        ${bid?`<div class="score" style="font-size:12px;min-width:0">📩 ${bid.team}<br>${bid.bid}万</div>
          <div style="display:flex;gap:4px"><button class="btn sm primary" style="margin:0" onclick="acceptBid(S,'${item.id}')">接受</button><button class="btn sm danger" style="margin:0" onclick="rejectBid(S,'${item.id}')">拒绝</button></div>`
  ...
  <div class="panel ${foldCls('mretired')}" data-fold="mretired"><h3><span class="h-ic">${ic('medal')}</span>退役名宿 <span class="tag">老将退役后 · 教练 / 主播</span></h3>
  ...
  const marketPanel=`<div class="panel ${foldCls('mmarket')}" data-fold="mmarket"><h3><span class="h-ic">${ic('case')}</span>自由市场 <span class="tag">${S.transferWindow>0?'转会窗开启·刷新免费·顶星增加':'刷新需5万/次'} · 每日特惠</span></h3>
  ...
  const minePanel=`<div class="panel ${foldCls('mine')}" data-fold="mine"><h3><span class="h-ic">${ic('users')}</span>我的队员</h3>
    ${S.players.length?`<div class="grid g4">${S.players.filter(p=>!S.lineup.includes(p.id)).map(p=>pcard(p,`<button class="btn sm primary" onclick="swapPlayer('${p.id}')">↑ 放入首发</button><div style="display:flex;gap:6px;margin-top:8px"><button class="btn sm danger" style="flex:1" onclick="openSellNego(S,'${p.id}')">💰 出售</button><button class="btn sm" style="flex:1" onclick="listPlayer(S,'${p.id}')">📋 挂牌</button></div>`)).join('')||'<div class="hint">全部队员都在首发阵容中</div>'}</div>`:'<div class="hint">还没有队员</div>'}
  </div>`;
...
function renderTrain(){
  let html=`<div class="panel"><h3><span class="h-ic">${ic('train')}</span>选手训练 <span class="tag">每天限1次 · 属性8万 / 英雄特训15万</span></h3>
  ...
  html+=`<div class="panel"><h3><span class="h-ic">${ic('star')}</span>青训营 <span class="tag">低薪高潜 · 工资帽友好</span></h3>
  ...
  html+=`<div class="panel"><h3><span class="h-ic">${ic('swap')}</span>位置改造 <span class="tag">30万 · 重构英雄池</span></h3>
...
function renderLeague(){
  ...
  const myG=myGroup(S);
  let html=`<div class="panel"><h3><span class="h-ic">${ic('chart')}</span>${PHASE_NAME[S.phase]||S.phase} <span class="tag">KPL 2025 官方赛制 · 18队 S/A/B</span></h3>
    <div class="hint" style="margin-bottom:8px">常规赛 BO5 全局BP · 胜者积1分 · 第一轮各组前2进S组 / 3-4进A组 / 5-6进B组 · 卡位赛 BO7 含巅峰对决 · 季后赛 10队双败</div></div>`;
  // 各组积分榜
  groups.forEach(g=>{
    ...
    html+=`<div class="panel"><h3>${g} 组 ${isMy?'<span class="tag" style="background:rgba(0,212,255,.15)">本队所在组</span>':''}</h3>
      ...
      ${rank.map((n,i)=>{
        ...
        return `<tr class="${n===S.teamName?'me':''}"><td>${i+1}</td><td>${n}${n===S.teamName?' ★':''}</td><td>${t.w}</td><td>${t.l}</td><td>${t.pts}</td><td>${t.pw}</td><td>${fmt(powerOf(S,n))}</td></tr>`;
      }).join('')}
      </table>
  ...
  if(S.eliminated&&S.eliminated.length){
    html+=`<div class="panel"><h3><span class="h-ic">${ic('ban')}</span>已淘汰</h3><div class="hint">${S.eliminated.join('、')}</div></div>`;
  }
  // 赛程
  if(S.schedule&&S.schedule.length){
    html+=`<div class="panel"><h3><span class="h-ic">${ic('cal')}</span>本队赛程（${PHASE_NAME[S.phase]}）</h3>${S.schedule.map(m=>{
      const cls=m.result==='W'?'win':m.result==='L'?'lose':'';
      const isNext=m===S.schedule[S.matchIdx];
      const oppIcon=(AI_TEAMS.find(t=>t.name===m.opp)||{}).icon||'⚔️';
      return `<div class="match ${cls}" ${isNext?'style="border-color:var(--cyan)"':''}>
        ...
        <div class="score" style="font-size:14px">${m.result?`${m.myScore}:${m.opScore}`:(isNext?'⬅️ 下一场':'待赛')}</div>
        <div class="vs" style="justify-content:flex-end;text-align:right"><div class="tname">${oppIcon} ${m.opp}</div><div class="power">战力 ${fmt(powerOf(S,m.opp))}</div></div>
      </div>`;
    }).join('')}</div>`;
  }
  // 卡位赛对阵（联赛页）
  if(S.phase==='card'&&S.card){
    html+=`<div class="panel"><h3><span class="h-ic">${ic('medal')}</span>卡位赛对阵 <span class="tag">BO7 · 含巅峰对决</span></h3>
      ${S.card.matches.map(m=>{
        const me=m.a===S.teamName||m.b===S.teamName;
  ...
  if(S.phase==='playoff'&&S.playoff){
    ...
    html+=`<div class="panel"><h3><span class="h-ic">${ic('trophy')}</span>季后赛对阵（BO7 双败 · 第7局巅峰对决）</h3>
...
function unionTeams(){
  ...
  names.forEach(n=>{
    if(n===S.teamName){
      ...
      teams.push({name:n,mine:false,icon:t?t.icon:'⚔️',roster:r,power:aiRosterPower(r)});
    }
  });
  return teams;
}
function renderUnion(){
  ...
  let html=`<div class="panel"><h3><span class="h-ic">${ic('shield')}</span>战队总览 <span class="tag">18 队 · 点击查看阵容</span></h3>
    ...
    ${teams.map((t,i)=>{
      ...
      return `<tr class="${t.mine?'me':''}" style="cursor:pointer" onclick="showSquad('${t.name}')">
        <td>${i+1}</td><td>${t.icon} ${t.name}${t.mine?' ★':''}${dst?` <span class="tag" style="color:var(--gold)">🏆${dst}连冠</span>`:''}</td>
  ...
  html+=`<div class="panel"><h3><span class="h-ic">${ic('star')}</span>选手榜单 <span class="tag">全联盟注册选手 TOP15</span></h3>
...
function showSquad(teamName){
  ...
  $('#app-modal-body').innerHTML=`
    <h2><span class="h-ic">${ic('shield')}</span>${icon} ${teamName} · 全队阵容 <span class="tag">${roster.length} 人</span></h2>
