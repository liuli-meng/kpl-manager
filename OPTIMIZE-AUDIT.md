...
## 第四轮（2026-09-18）：见 `OPTIMIZE-AUDIT-2.md`

本轮用真实存档重量，**推翻了本报告三处结论**，细节与复现方式都在 `OPTIMIZE-AUDIT-2.md`：

1. **P1-1「转会页已修至 28.6 KB / 604 元素」在真实存档下不成立** —— 实测 123.6 KB / 5,023 个标签。差异不在渲染函数，而在**列表规模**：`transferList` 89 人全量出行、选秀池 20 人全量出 `pcard`。已用「按需展开 + 非本队回合紧凑行」降到 55.7 KB / 2,169（浏览器实测 906 个元素节点）。并新增常驻门禁 `tests/verify-render-size.js` 钉住体积，防止长回去。
2. **P1-2「读档手机白屏 100–250 ms」与本报告的反驳「0.25 ms」都不成立** —— 两个数都没区分 V8 冷编译与热路径。真实口径：冷启动首次 `load()` 48 ms，热身 6 次后中位 **5.1 ms**。结论仍是「不用改异步」，但理由换了。
3. **P2 里隐含的「索引化能提速」不成立**（也是 `kpl-manager-rewrite` 线的头号收益）—— `s.players` 实测只有 8 人，最长的被扫数组是 `extraDefs`(139)；A/B 四种变体的读档与渲染耗时全落在噪声里。**建议放弃 rewrite 的整体合回**，只单独移植「常规赛 schedule 进 `mid`」（它正好是 P2-7 的正解）。

另外：第五节「新发现」里的行尾混态已加 `.gitattributes` 处理；实测工作区 CRLF 文件远不止 9 个（`ui.js` 是 1338/1338 行全 CRLF），但因 `core.autocrlf=true` 索引侧本就存 LF，加该文件**不产生改档噪音**（已用 `git status` 验证）。

本节遗留待拍板的两项（`history.logs` 15%、转会窗 37%）**建议直接关闭**：存档 145 K 字符 × 4 槽 + 自动备份 ≈ 1 MB，离 localStorage 上限很远，brotli 后传输也不是瓶颈——省这 15% 要付迁移与旧档兼容的代价，不值。
